...
from .forms import ModuleFormSet, SearchForm
...
class CourseModuleUpdateView(TemplateResponseMixin, View):
    ...
    def get(self, request, pk, *args, **kwargs):
        ...
        return self.render_to_response({'course': course,'formset': formset})
...
def course_search(request):   
    ...
    if request.method == "GET":
        ...
        results = Course.objects.annotate(search=search_vector,
                    rank=SearchRank(search_vector, search_query)).filter(
                                        search=search_query).order_by('-rank')
        return render(request,'courses/course/search.html',{'query': query,'results': results})